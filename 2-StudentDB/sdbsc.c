...
int get_student(int fd, int id, student_t *s)
{
    if (id < MIN_STD_ID || id > MAX_STD_ID) {
        return ERR_DB_FILE; // Invalid ID range
    }
    ...
    if (bytes_read == 0 || s->id == DELETED_STUDENT_ID) {
        return SRCH_NOT_FOUND; // Student not found or deleted
    }

    return NO_ERROR; // Successfully found student
}
/*
 *  add_student
 *      fd:     linux file descriptor
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    if (id < MIN_STD_ID || id > MAX_STD_ID || gpa < MIN_STD_GPA || gpa > MAX_STD_GPA) {
        return ERR_DB_OP; // Invalid ID or GPA range
    }

    int offset = id * sizeof(student_t);

    student_t existing_student;
    int status = get_student(fd, id, &existing_student);
    ...
    if (status == NO_ERROR) {
        printf(M_ERR_DB_ADD_DUP, id); // Student already exists
        return ERR_DB_OP;
    }
    ...
    // Seek again to ensure proper writing position
    if (lseek(fd, offset, SEEK_SET) == -1) {
        return ERR_DB_FILE; // Seek error
    }
...
int del_student(int fd, int id)
{
    ...
    // Write an empty student record (deleting the student)
    ...
    return NO_ERROR; // Successfully deleted student
}
...
int count_db_records(int fd)
{
   if (fd < 0) {
	...
	while(1)	{
		bytesRead = read(fd, &record, STUDENT_RECORD_SIZE);

		if (bytesRead == 0) {
			break;
		}

		if (bytesRead != STUDENT_RECORD_SIZE || bytesRead < 0){
			printf(M_ERR_DB_READ);	
			return ERR_DB_FILE;
		}
 ...
 *			print("a")i  on the first real row encountered print the header for the required output:
 *
...
int print_db(int fd) {
	student_t student;
   int header_printed = 1;

    // Loop through the database and read each student record
    while (1) {
        ssize_t bytes_read = read(fd, &student, sizeof(student_t));
        if (bytes_read == -1) {
            printf(M_ERR_DB_READ);
            return ERR_DB_FILE; // Error reading the file
        }

        if (bytes_read == 0) {
            break; // End of file reached
        }
		  //function below is to check that the database is empty

        // Check if the record is empty or deleted
        if (memcmp(&student, &EMPTY_STUDENT_RECORD, sizeof(student_t)) != 0) {
            // If the header has not been printed yet, print it
            if (header_printed == 1) {
                printf(STUDENT_PRINT_HDR_STRING, "ID", "FIRST_NAME", "LAST_NAME", "GPA");
                header_printed = 0; // Mark the header as printed
            }

            // Calculate GPA as a float (gpa stored as an integer, e.g., 450 means 4.50)
    ...
    // If no records were found, print a message indicating the database is empty
...
{
    // Check if the student pointer is NULL or if the student id is 0
    if (s == NULL || s->id == DELETED_STUDENT_ID) {
        printf(M_ERR_STD_PRINT);
        return; // Exit the function if invalid
    }

    // Print the header for the student table
    printf(STUDENT_PRINT_HDR_STRING, "ID", "FIRST_NAME", "LAST_NAME", "GPA");

    // Calculate GPA as a float a is stored as an integer
...
int compress_db(int fd)
{
    // Close the original database file (since we will overwrite it)
    close(fd);
    ...
    // Open/Create the temporary database file
    ...
    // Read all records from the original database
    while (read(fd, &student, sizeof(student_t)) == sizeof(student_t)) {
        // Copy only non-empty records to the temporary database
        if (memcmp(&student, &EMPTY_STUDENT_RECORD, sizeof(student_t)) != 0) {
    ...
    // Reopen the compressed database file to return its descriptor
    ...
    return fd; // Return new database file descriptor
}
