...
#include "dshlib.h"
#include "rshlib.h"
//Arslan Sanabayev, as5764, CS283 6-RShell
...
int exec_remote_cmd_loop(char *address, int port) {
    ...
    while (1) {
        ...
        while (1) {
            ssize_t io_size = recv(cli_socket, rsp_buff, RDSH_COMM_BUFF_SZ - 1, 0);
            if (io_size < 0) {
        ...
        if (strstr(cmd_buff, "exit") || strstr(cmd_buff, "stop-server")) {
            break;
        }
    }
